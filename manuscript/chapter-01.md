...
```mermaid
timeline
    title Tiến hóa kiến trúc phần mềm doanh nghiệp
    section 1990s
        Monolith : Ứng dụng nguyên khối
                 : Đơn giản, nhanh, dễ debug
                 : Giới hạn ở scale & team size
    section 2000s
        SOA : Service-Oriented Architecture
            : ESB, WSDL, SOAP
            : Tái sử dụng ở cấp enterprise
            : Phức tạp, nặng middleware
    section 2010s
        Microservices : Kiến trúc vi dịch vụ
                      : REST, messaging, containers
                      : Independence-first
                      : Netflix, Amazon, Spotify tiên phong
    section 2020s
        Evolved MS : Domain-oriented MS (DOMA)
                   : Modular monolith comeback
                   : Hybrid approaches phổ biến
                   : Fast flow success triangle
```
...
Erl đúc kết 8 nguyên lý nền tảng cho mọi dịch vụ. Bảng dưới đây cho thấy microservices kế thừa và chuyển hóa chúng như thế nào:
...
```mermaid
graph TB
    subgraph SOEA["Service-Oriented Enterprise Architecture"]
        subgraph SIA["Service Inventory Architecture"]
            subgraph SCA["Service Composition Architecture"]
                SA["Service Architecture"]
            end
        end
    end
    
    SA --> |"Một dịch vụ đơn lẻ"| note1["Microservices tập trung ở đây"]
    SCA --> |"Tổ hợp nhiều dịch vụ"| note2["Saga, API Gateway"]
    SIA --> |"Danh mục toàn bộ dịch vụ"| note3["Service Registry, Discovery"]
    SOEA --> |"Toàn doanh nghiệp"| note4["SOA truyền thống hướng tới cấp này"]
    
    style SA fill:#4CAF50,color:white
    style SCA fill:#2196F3,color:white
    style SIA fill:#FF9800,color:white
    style SOEA fill:#9C27B0,color:white
```
...
Đặc biệt, điều kiện thứ tư (*independent deployability*) chính là thước đo quan trọng nhất. Nếu bạn không thể deploy một service mà không phải deploy các service khác — bạn chưa có microservices thực sự.
...
```mermaid
graph LR
    subgraph X["X-axis: Horizontal Cloning"]
        X1["Instance 1"]
        X2["Instance 2"]
        X3["Instance N"]
    end
...
Uber bắt đầu như một monolith Python đơn giản, phát triển chóng mặt, và khoảng 2014–2016 chuyển sang microservices. Tuy nhiên, câu chuyện Uber thú vị ở phần *hậu chuyển đổi*: đến khoảng 2020, với hơn 2.000 microservices, họ nhận ra mình đang rơi vào *"microservice madness"* — quá nhiều service nhỏ tạo ra mạng lưới phụ thuộc phức tạp, khó debug và vận hành.
...
```mermaid
graph LR
    A["Monolith đạt giới hạn"] --> B{"Scaling truyền thống<br/>đáp ứng được?"}
    B -->|Có| C["Vertical/Horizontal scaling<br/>Giữ monolith"]
    B -->|Không| D{"Team > 10 người?<br/>Multi-team?"}
    D -->|Không| E["Modular Monolith"]
    D -->|Có| F["Microservices<br/>(thường bắt đầu hybrid)"]
    F --> G{"Quá nhiều services nhỏ?"}
    G -->|Có| H["Domain-Oriented MS<br/>(Uber's DOMA)"]
    G -->|Không| I["Tiếp tục tiến hóa"]
    
    style A fill:#EF5350,color:white
    style C fill:#66BB6A,color:white
    style E fill:#42A5F5,color:white
    style F fill:#AB47BC,color:white
    style H fill:#FFA726,color:white
```
...
```mermaid
graph LR
    PM["Patchwork Monolith<br/><i>Big ball of mud</i>"] -->|"Refactor nội bộ<br/>(đây là bước khó nhất)"| MM["Modular Monolith<br/><i>Clear boundaries</i>"]
    MM -->|"Tách service<br/>(khi thực sự cần)"| MS["Microservices<br/><i>(hybrid hoặc full)</i>"]
    
    PM -.->|"❌ Tách trực tiếp<br/>= distributed monolith"| DM["Distributed Monolith<br/><i>Worst of both worlds</i>"]
    
    style PM fill:#EF5350,color:white
    style MM fill:#66BB6A,color:white
    style MS fill:#42A5F5,color:white
    style DM fill:#212121,color:white
```
...
## 1.6 Khi nào nên — và không nên — dùng Microservices
...
```mermaid
flowchart TD
    Q1{"1. Vấn đề bạn đang<br/>giải quyết là gì?"} -->|"Cần independent deploy,<br/>team autonomy, selective scale"| Q2
    Q1 -->|"Cần code sạch hơn,<br/>performance tốt hơn"| N1["❌ Microservices không<br/>giải quyết vấn đề này.<br/>Xem xét refactor."]
    
    Q2{"2. Đã thử cách<br/>đơn giản hơn?"} -->|"Modular monolith<br/>đã không đủ"| Q3
    Q2 -->|"Chưa thử"| N2["⏸ Thử modular monolith<br/>trước. Ít rủi ro hơn."]
    
    Q3{"3. Team có đủ năng lực<br/>vận hành?"} -->|"Có kinh nghiệm<br/>distributed systems"| OK["✅ Xem xét<br/>microservices"]
    Q3 -->|"Chưa có"| N3["📚 Đầu tư đào tạo<br/>DevOps/distributed trước"]
    
    style Q1 fill:#1565C0,color:white
    style Q2 fill:#1565C0,color:white
    style Q3 fill:#1565C0,color:white
    style OK fill:#2E7D32,color:white
    style N1 fill:#C62828,color:white
    style N2 fill:#EF6C00,color:white
    style N3 fill:#EF6C00,color:white
```
...
>
> Nếu chia hệ thống thành nhiều "service" nhưng chúng vẫn phải deploy cùng nhau, chia sẻ database, và thay đổi đồng loạt — bạn có một **distributed monolith**: mang toàn bộ nhược điểm của monolith *VÀ* distributed system, không có ưu điểm nào. Đây là kết quả phổ biến nhất khi tách service mà chưa modular hóa.
...
graph TB
    ...
    subgraph Cloud["LMS Backend"]
        ...
        GW --> AUTH["🔑 Auth Service<br/>Authentication,<br/>User Management"]
        GW --> APP["📝 Core Service<br/>Platform,<br/>SQL Practice"]
        GW --> ASN["📚 Assignment Service<br/>Assignments,<br/>Exams, Grading"]
        
        APP -->|"Kafka"| JUDGE["⚖ Judge Service<br/>SQL Execution,<br/>Auto-grading"]
        APP -.->|"WebSocket"| NOTIF["🔔 Notification Service<br/>Real-time alerts"]
    ...
    subgraph Data["Data Layer"]
        ...
        SANDBOX["🗄 MySQL / MSSQL<br/>(Student sandboxes)"]
        KAFKA["📨 Apache Kafka"]
    end
...
Mỗi chương sách sẽ quay lại hệ thống LMS ở góc nhìn khác nhau. Bảng dưới đây ánh xạ các bài toán kỹ thuật — từ hiện trạng đến hướng migration:
...
| **Testing** | Chưa có test strategy | Cần unit/integration/contract tests | Ch.10 |
...
3. [2b] Chris Richardson, *Microservices Patterns*, 2nd Ed. — Ch.1: Architecture for Fast Flow
