...
└── references/                    ← Git SUBMODULE → ptit-microservice-textbook-internal (PRIVATE)
    ...
    │   ├── scripts/               ← Build pipeline (build-typst.ps1, gen_svg_chNN.py...)
    │   └── typst/                 ← Typst source (main.typ, main-web.typ, chapters/, theme/, filters/)
...
- All build scripts (`build-typst.ps1`, `gen_svg_ch01.py`…`gen_svg_ch12.py`, `svg_design_system.py`)
...
### Step 5 — Build PDF/HTML (Author only)

> [!IMPORTANT]
> **Diagram Pipeline Rule**: Active book figures are authored as HTML sources in `figures/chNN/fig-N-N.html` and captured to ignored PNG artifacts by `references/internal/scripts/gen-diagrams.ps1`. Manuscript image references should point to `.png`, not `.svg`. Legacy Python SVG generators are retained only as fallback/archive and `build-typst.ps1` runs them only if an active manuscript reference still points to `.svg`.
> 
> **Diacritics Rule**: Trước khi commit legacy SVG changes, chạy `audit-svg-diacritics.py` để kiểm tra text tiếng Việt có dấu. Release build tự động chạy audit và block nếu có violations.
...
```powershell
# Dev PDF build (resets output/; gitignored)
...
Back matter: `exercises.md`, `appendix-*.md` (A: Glossary, B: Tools & Resources, C: Pattern Catalog, D: Anti-patterns)
