...
> [!IMPORTANT]
> **SVG Regeneration Rule**: All SVG diagrams are generated by Python scripts (`gen_svg_ch01.py` … `gen_svg_ch12.py`) using the shared `svg_design_system.py` v2. The build script `build-typst.ps1` tự động chạy tất cả gen scripts trước khi compile PDF — không cần chạy tay. Scripts are named `gen_svg_chNN.py` (no `_new` suffix) — the legacy `_new` suffix was used during migration and has been removed.
> 
> **Diacritics Rule**: Trước khi commit SVG, chạy `audit-svg-diacritics.py` để kiểm tra text tiếng Việt có dấu. Release build tự động chạy audit và block nếu có violations.
