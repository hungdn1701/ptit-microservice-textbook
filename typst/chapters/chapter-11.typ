...
#box(image("/figures/ch11/fig-11-1.svg"))

#emph[Hình 11.1: Request flow qua 5+ services --- lỗi xảy ra ở đâu?]
...
#box(image("/figures/ch11/fig-11-2.svg"))

#emph[Hình 11.2: Ba trụ cột của Observability --- Logs, Metrics, Traces]

#strong[Bảng 11.1:] Vai trò của ba trụ cột Observability
...
#strong[Bảng 11.2:] Observability vs Monitoring
...
#strong[Bảng 11.3:] Observability Maturity Model
...
#box(image("/figures/ch11/fig-11-3.svg"))

#emph[Hình 11.3: Logs phân tán (manual debug) vs Logs tập trung --- Search một nơi]
...
#emph[Hình 11.4: Pipeline gom logs tập trung --- từ service đến storage] #box(image("/figures/ch11/fig-11-4.svg"))

Hai stack phổ biến nhất:

#strong[Bảng 11.4:] So sánh hai stack ELK và PLG
...
#box(image("/figures/ch11/fig-11-5.svg"))

#emph[Hình 11.5: Truyền Correlation ID xuyên suốt request, kể cả qua Kafka]
...
#box(image("/figures/ch11/fig-11-6.svg"))

#emph[Hình 11.6: Gantt chart của một Trace --- hiển thị timing cho từng Span]
...
#strong[Bảng 11.5:] Các khái niệm cơ bản trong Distributed Tracing
...
#strong[Bảng 11.6:] Sampling strategies cho Tracing
...
#strong[Bảng 11.7:] Bốn loại metrics cần thu thập
...
#strong[Bảng 11.8:] Framework RED và USE
...
#strong[Bảng 11.9:] Sự khác biệt giữa SLI, SLO và SLA
...
#box(image("/figures/ch11/fig-11-7.svg"))

#emph[Hình 11.7: Mối quan hệ giữa SLI, SLO và SLA]

Với LMS --- hệ thống giáo dục nội bộ --- SLO phù hợp:

#strong[Bảng 11.10:] SLO đề xuất cho hệ thống LMS
...
#strong[Bảng 11.11:] Chiến lược Alerting theo mức độ nghiêm trọng
...
#strong[Bảng 11.12:] Taxonomy hệ thống mã lỗi (Error Code)
...
#strong[Bảng 11.13:] Ba loại Health Checks
...
#box(image("/figures/ch11/fig-11-8.svg"))

#emph[Hình 11.8: User Activity Tracking pipeline qua Kafka]

#strong[Bảng 11.14:] Ứng dụng của User Activity Tracking
...
#strong[Bảng 11.15:] Phân loại Failure Injection trong Chaos Engineering
...
#strong[Bảng 11.16:] Chaos experiments cho hệ thống LMS
...
#strong[Bảng 11.17:] Netflix Simian Army
...
#box(image("/figures/ch11/fig-11-9.svg"))

#emph[Hình 11.9: Đánh giá hiện trạng Observability của LMS]
...
#strong[Bảng 11.18:] Mức độ trưởng thành observability phân theo component
...
#strong[Bảng 11.19:] Hai chế độ hoạt động chính của LMS
