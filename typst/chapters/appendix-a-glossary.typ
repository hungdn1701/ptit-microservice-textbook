// Auto-converted: manuscript/appendix-a-glossary.md â†’ Typst
// Converted: 2026-04-29
// REVIEW: Verify callout box titles and table captions.
#import "../components/compat.typ": *
= Phụ lục A: Bảng thuật ngữ
#quote(block: true)[
Thuật ngữ kỹ thuật sử dụng trong sách, sắp xếp theo bảng chữ cái. Sync với #link("style-guide.md") Section 2.
]

#line(length: 100%, stroke: 0.5pt + rgb("#E5E7EB"))

#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([English], [Cách dùng trong sách], [Chương đầu],),
    table.hline(),
    [Aggregate], [Aggregate], [Ch.2],
    [API Gateway], [API Gateway], [Ch.8],
    [Blue/Green Deployment], [Blue/Green deployment], [Ch.12],
    [Bounded Context], [Bounded Context], [Ch.2],
    [Bulkhead], [Bulkhead], [Ch.4],
    [Canary Deployment], [Canary deployment], [Ch.12],
    [Circuit Breaker], [Circuit Breaker], [Ch.4],
    [Choreography], [choreography], [Ch.6],
    [CI/CD], [CI/CD], [Ch.12],
    [Container], [container], [Ch.12],
    [Correlation ID], [correlation ID], [Ch.8],
    [CQRS], [CQRS], [Ch.7],
    [Dead Letter Topic/Queue], [Dead Letter Topic (DLT)], [Ch.5],
    [Docker Compose], [Docker Compose], [Ch.12],
    [Domain-Driven Design], [DDD (Domain-Driven Design)], [Ch.2],
    [Event Sourcing], [Event Sourcing], [Ch.7],
    [Event Storming], [Event Storming], [Ch.5],
    [Event-driven], [hướng sự kiện (event-driven)], [Ch.5],
    [Idempotency], [idempotency / idempotent], [Ch.4],
    [Infrastructure as Code], [IaC (Infrastructure as Code)], [Ch.12],
    [JWT], [JWT (JSON Web Token)], [Ch.9],
    [Load Balancing], [load balancing], [Ch.4],
    [Message broker], [message broker], [Ch.5],
    [Microservice], [microservice / dịch vụ vi mô (lần đầu)], [Ch.1],
    [OAuth2], [OAuth2], [Ch.9],
    [Observability], [Observability], [Ch.11],
    [OpenFeign], [OpenFeign], [Ch.4],
    [Orchestration], [orchestration], [Ch.6],
    [Rate Limiting], [rate limiting], [Ch.8],
    [RBAC], [RBAC (Role-Based Access Control)], [Ch.9],
    [Saga pattern], [Saga pattern], [Ch.6],
    [Service], [service], [Ch.1],
    [Service Discovery], [Service Discovery], [Ch.4],
    [SLI/SLO/SLA], [SLI, SLO, SLA], [Ch.11],
    [Strangler Fig], [Strangler Fig pattern], [Ch.10],
  )]
  , kind: "inline-table", supplement: none, numbering: none
  )

#line(length: 100%, stroke: 0.5pt + rgb("#E5E7EB"))

#quote(block: true)[
#strong[Lưu ý:] Lần đầu xuất hiện trong text, thuật ngữ được in nghiêng kèm giải thích tiếng Việt. Sau đó dùng thuật ngữ tiếng Anh trực tiếp.
]
