...
#strong[Bảng 10.1:] Decision Matrix --- khi nào cân nhắc migration
...
#strong[Bảng 10.1b:] Anti-patterns khi chuyển đổi Microservices (minh họa với LMS)
...
#strong[Bảng 10.2:] Big Bang vs Incremental migration
...
#box(image("/figures/ch10/fig-10-1.svg"))

#emph[Hình 10.1: Strangler Fig Pattern --- dần thay thế monolith qua 3 giai đoạn]

=== API Gateway --- "Cây đa" trong microservices
API Gateway (đã học ở Ch.8) đóng vai trò tự nhiên làm #strong[seam] cho Strangler Fig:

#box(image("/figures/ch10/fig-10-2.svg"))

#emph[Hình 10.2: API Gateway làm seam cho Strangler Fig --- client không biết route nào đến monolith, route nào đến service mới]

#strong[Bảng 10.3:] Ba implementation strategies cho Strangler Fig
...
#box(image("/figures/ch10/fig-10-3.svg"))

#emph[Hình 10.3: Shared Database (coupled) vs Database-per-Service (decoupled)]
...
#strong[Bảng 10.4:] 5 chiến lược tách database --- từ góc migration
...
#box(image("/figures/ch10/fig-10-4.svg"))

#emph[Hình 10.4: Thứ tự thực hiện 5 chiến lược tách database]
...
#box(image("/figures/ch10/fig-10-5.svg"))

#emph[Hình 10.5: Dual-Write Pitfall --- DB thành công nhưng event thất bại]
...
#box(image("/figures/ch10/fig-10-6.svg"))

#emph[Hình 10.6: Outbox Pattern --- event ghi cùng transaction với business data]

#strong[Bảng 10.5:] Hai cách implement Outbox Pattern
...
#box(image("/figures/ch10/fig-10-7.svg"))

#emph[Hình 10.7: Anti-Corruption Layer --- lớp dịch giữa service mới và legacy]
...
#box(image("/figures/ch10/fig-10-8.svg"))

#emph[Hình 10.8: Branch by Abstraction --- migration an toàn qua feature flag]
...
#box(image("/figures/ch10/fig-10-9.svg"))

#emph[Hình 10.9: Parallel Run --- chạy song song old và new logic, so sánh output]
...
#strong[Bảng 10.6:] Tổng hợp Gap Analyses xuyên suốt Ch.1--12
...
#box(image("/figures/ch10/fig-10-10.svg"))

#emph[Hình 10.10: Migration Roadmap 4 phases --- Quick Wins → Observability → Resilience → Decomposition]

==== Phase 1 --- Quick Wins (Effort thấp, Impact cao)
#strong[Bảng 10.7:] Phase 1 --- Quick Wins
...
#strong[Bảng 10.8:] Phase 2 --- Observability Foundation
...
#strong[Bảng 10.9:] Phase 3 --- Resilience & CI/CD
...
#strong[Bảng 10.10:] Phase 4 --- Database Decomposition
...
#box(image("/figures/ch10/fig-10-11.svg"))

#emph[Hình 10.11: Decision Matrix --- ưu tiên theo Impact × Effort]
