...
- Sử dụng OpenFeign để gọi service-to-service một cách khai báo (declarative)
...
| **Ecosystem** | Rất rộng | Đang phát triển nhanh |

```protobuf
// Ví dụ: Judge Service contract bằng Protocol Buffers
syntax = "proto3";

service JudgeService {
  rpc Execute(JudgeRequest) returns (JudgeResult);
  rpc StreamResults(JudgeRequest) returns (stream JudgeResult); // streaming!
}
...
String url = "http://judge-service:8082/api/judge/execute";
...
Vấn đề: URL hardcoded, header management thủ công, error handling lặp lại ở mỗi call. Khi có 10+ API calls, code trở nên khó bảo trì.
...
public interface MysqlClient {
    ...
    JudgeResult execute(@RequestBody JudgeRequest request);
...
public class SqlExecutorService {
    ...
    public JudgeResult executeQuery(JudgeRequest request) {
        return switch (request.getDatabaseType()) {
            case MYSQL -> mysqlClient.execute(request);
            case SQLSERVER -> sqlServerClient.execute(request);
            // ...
        };
    }
}
...
Khi microservice khởi động, nó đăng ký tại Eureka server với tên (`spring.application.name`) và địa chỉ. Gateway và các service khác query Eureka để tìm instances.

Trong API Gateway config (Spring Cloud Gateway):
```yaml
spring:
  cloud:
    gateway:
...
`lb://core-service` nghĩa là: tra cứu Eureka tìm tất cả instances có tên `core-service`, rồi load balance giữa chúng.
...
**1. Timeout** — Đặt giới hạn thời gian chờ. Không bao giờ chờ vô hạn.

```java
@FeignClient(name = "judge-mysql")
public interface MysqlClient {
    @PostMapping("/api/execute")
    JudgeResult execute(@RequestBody JudgeRequest request);
}

// Config: feign.client.config.judge-mysql.connectTimeout=2000
// Config: feign.client.config.judge-mysql.readTimeout=5000
```

Best practice: timeout nên ngắn hơn bạn nghĩ. 30 giây timeout = 30 giây thread bị block = 30 giây user chờ. Netflix đặt timeout chuẩn 1–3 giây cho internal calls [W3].

**2. Retry** — Tự động thử lại khi gặp lỗi tạm thời (transient failures):
...
public JudgeResult executeWithRetry(JudgeRequest request) {
    return mysqlClient.execute(request);
}
```

Lưu ý quan trọng: **chỉ retry idempotent operations** (GET, PUT, DELETE). Retry POST có thể tạo duplicate data. Richardson trong [2a, Ch.3] nhấn mạnh: retry token (idempotency key) là bắt buộc khi retry non-idempotent operations.

**3. Circuit Breaker** — "Cầu dao" tự động ngắt khi service downstream liên tục lỗi:
...
**4. Bulkhead** — Cách ly resources theo service/function. Nếu thread pool cho Judge Service đầy, Core Service vẫn phục vụ được các request không liên quan đến Judge:
...
Trong thực tế, các patterns thường kết hợp:

```
Request → Timeout (3s) → Retry (3 lần, backoff) → Circuit Breaker → Bulkhead → Actual Call
```

Spring Cloud Circuit Breaker + Resilience4j là stack phổ biến nhất cho Java microservices.
...
```java
// Strategy pattern: dispatch dựa trên database type
public JudgeResult execute(String sql, UUID databaseTypeId) {
    if (databaseTypeId.equals(MYSQL_UUID)) {
        return mysqlClient.execute(new JudgeRequest(sql));
    } else if (databaseTypeId.equals(SQLSERVER_UUID)) {
        return sqlServerClient.execute(new JudgeRequest(sql));
    }
    // ... PostgreSQL, Oracle handled locally
}
```
...
**Phase 1 — Thêm resilience** (ưu tiên cao, effort thấp):
```java
// Thêm Circuit Breaker + Timeout cho Feign calls
@CircuitBreaker(name = "judge-mysql", fallbackMethod = "judgeFallback")
@Retry(name = "judge-mysql")
@TimeLimiter(name = "judge-mysql")
public JudgeResult executeMysql(JudgeRequest request) {
    return mysqlClient.execute(request);
}

public JudgeResult judgeFallback(JudgeRequest request, Throwable t) {
    return JudgeResult.pending("Sandbox đang bận, bài sẽ được chấm khi sẵn sàng");
}
```

**Phase 2 — Hợp nhất SqlExecutorService** (effort trung bình):
- Chuyển toàn bộ execution logic sang Judge Service (single ownership)
- Core Service chỉ gửi request, không biết database type nào xử lý

**Phase 3 — Chuyển execution sang async** (effort cao, giá trị lớn):
- Submit → Kafka → Judge → Result event → Core cập nhật
- User không chờ đồng bộ — nhận notification khi có kết quả
- Đây chính là pattern mà LMS đã bắt đầu implement (Chương 5)
...
2. [2b] Chris Richardson, *Microservices Patterns*, 2nd Ed. — Ch.3: Communication Patterns
3. [4a] Sam Newman, *Building Microservices* — Ch.4: Integration, Smart Endpoints and Dumb Pipes
4. [5] Hugo Rocha, *Practical Event-Driven MS Architecture* — Ch.7: Resilience & Reliability
