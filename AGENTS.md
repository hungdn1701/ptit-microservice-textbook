...
- All build scripts (`build-typst.ps1`, `gen_svg_*.py`, `svg_design_system.py`)
...
> [!IMPORTANT]
> **SVG Regeneration Rule**: All SVG diagrams are generated by Python scripts (`gen_svg_ch*_new.py`) using the shared `svg_design_system.py` v2. The build script `build-typst.ps1` tự động chạy tất cả gen scripts trước khi compile PDF — không cần chạy tay.
> 
